...
def Extract_rec(pdf_path,middle_path):
    ...
    text_path = middle_path + "/" + filename + "/" + "original_parts.json"
    ref_path = middle_path + "/" + filename + "/" + "Reference.json"
    #执行操作
...
def Extract(pdf_path, out_file, details):
    ...
    if not os.path.exists(out_file + "/" + filename):
        os.makedirs(out_file + "/" + filename)
    output_file_path_txt = out_file + "/" + filename + "/" + "text.txt"
    output_file_path_json = out_file + "/" + filename + "/" + "text_parts.json"
    output_origin_file_path_json = out_file + "/" + filename + "/" + "original_parts.json"
...
def Insert_Images(pdf_path, middle_path,model="gpt-4-32k",insert_image = True,reference = True,trans = False,):
    ...
    text_path = middle_path + "/" + filename + "/text_parts.json"
    images_path = middle_path + "/" + filename+ "/" + "images"
    images_information_path = middle_path + "/"+ filename+ "/"+ "images_information.json"
    store_path = middle_path + "/"+filename + "/" + "result_{}.docx".format(filename)
...
if __name__ == "__main__":
    ...
    Single_Process(pdf_path,middle_path,mode=0,insert_image=False,reference=False,trans=False,model="gpt-4-turbo")
    '''
        :param pdf_path: pdf存储路径
        :param middle_path: 中间文件存储文件夹
        :param mode: 模式选择，0为简单版本，1为详细版本
