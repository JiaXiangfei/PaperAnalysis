...
from backend.fic_tools_sdk.config import OSS_Config
from backend.fic_tools_sdk.fic_tools import File_Tools, Tools
...
class ObjectStor_Handler:
    ...
    def create_minio_client(self):
        ...
        try:
            return Minio(
                ...
                secure=self.config.https
            )
        except Exception as err:
            error_message = f'create_minio_client|Exception error: {str(err)}'
            return None
        except ResponseError as err:
            error_message = f'create_minio_client|ResponseError error: {str(err)}'
        except InvalidResponseError as err:
            error_message = f'create_minio_client|InvalidResponseError error: {str(err)}'
    ...
    def upload_file(self, folder_name, folder_path):
        ...
        try:
            ...
            raise ResponseError(f'def upload_file|File: {self.bucket_name} upload_failed: {str(err)}')
    ...
    def upload_files_to_minio(self, folder_name, upload_dir):
        ...
        # 使用工具方法从目录上传文件
        File_Tools.upload_files(self.upload_single_file, folder_name, upload_dir)

    def minio_folder_list(self, bucket_name=None):
        """
    ...
    def minio_list_by_prefix(self, bucket_name, prefix):
        """
        获取MinIO存储桶中指定前缀下的所有对象（文件或文件夹）的名称列表。
        """
        list_object_names: list[str | None] = []
        for obj in self.minioClient.list_objects(bucket_name, prefix=prefix, recursive=True):
            # 判断对象是否为文件
            try:
                list_object_names.append(obj.object_name)
            except ResponseError as err:
                # 打印删除过程中遇到的错误
                print(f"输出前缀下的对象时出错： {obj.object_name}: {err}")
        return list_object_names

    def create_minio_multi_object_url(self, prefix):
        """
        生成一个用于访问MinIO服务器上指定前缀对象的URL。
        """
        list_object_names = []
        list_object_names = self.minio_list_by_prefix(self.bucket_name, prefix)
        list_url = []

        expiration_time = timedelta(seconds=100)

        for obj in list_object_names:
            # 判断对象是否为文件
            try:
                if not obj.endswith("/"):
                    presigned_url = self.minioClient.presigned_get_object(self.bucket_name, obj,
                                                                          expires=expiration_time)
                    list_url.append(presigned_url)
                    print(f"临时URL（有效10秒）: {presigned_url}")
            except Exception as e:
                print(f"生成临时URL时发生错误: {e}")
                return None
    ...
    def download_file_from_minio_by_prefix(self, object_name, file_path, bucket_name=None):
        if bucket_name is None:
            bucket_name = self.bucket_name
        try:
    ...
    def create_minio_single_object_url(self, object_name):
        """
        生成一个用于访问MinIO服务器上指定对象的URL。
        """
        ...
        try:
            # 生成临时URL
            presigned_url = self.minioClient.presigned_get_object(self.bucket_name, object_name,
                                                                  expires=expiration_time)
            print(f"临时URL（有效10秒）: {presigned_url}")
    ...
    def is_file_exist(self, bucket_name, file_md5, folder_md5):
        ...
        try:
            ...
            # 记录S3访问错误。
            raise S3Error(f'S3Error error: {str(err)}')
    ...
    def is_minio_bucket_exist(self):
        ...
        try:
            ...
            print(f'Bucket: {self.bucket_name} is exist')
        except InvalidResponseError as err:
            # 记录InvalidResponseError异常错误日志
            raise InvalidResponseError(f'S3Error error: {str(err)}')
    ...
    def delete_minio_bucket(self):
        ...
        try:
            ...
            raise InvalidResponseError(f'InvalidResponseError error: {str(err)}')  # 记录错误日志
    ...
    def minio_file_etag_list(self, bucket_name, prefix):
        ...
        try:
            ...
            # 记录遇到的S3异常错误
            raise S3Error(f'Unexpected error: {str(err)}')

    def get_object_name_by_prefix(self, bucket_name, prefix):
        """
        获取MinIO存储桶中指定前缀的文件的名称列表。
        ...
        try:
            ...
            for obj in objects:
                # 将每个对象的名称添加到列表中
                list_object_name.append(obj.object_name)

            return list_object_name[0]
        except S3Error as err:
