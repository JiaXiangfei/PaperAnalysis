...
def MatchImage(text, image_info, image_path, store_path, model="gpt-4-32k", insert_image=True, reference=True, trans=True):
    ...
    if insert_image:
        ...
        with open(os.path.join(parent_directory,"images_insert.txt"), 'w',encoding="utf-8") as temp_file:
    ...
    # 翻译
    if trans:
        images_insert_txt_path = os.path.join(parent_directory, "images_insert.txt")
        images_insert_json_path = os.path.join(parent_directory, "images_insert.json")
        image_tag = r"<(\d+)>"
        result_cn = translate(text, images_insert_txt_path, images_insert_json_path, image_tag)
        with open(os.path.join(parent_directory, "images_insert_cn.json"), 'w', encoding="utf-8") as json_file:
            json.dump(result_cn, json_file, ensure_ascii=False, indent=4)
        response = format_json_string(os.path.join(parent_directory, "images_insert_cn.json"))

    # 拼接reference
    if reference:
        if os.path.exists(os.path.join(parent_directory, "Reference.json")):
            # with open(os.path.join(parent_directory, "Reference.json"), 'r', encoding="utf-8") as reference_file:
            #     reference = reference_file.read()
            # full_text = response + "\n" + reference
            with open(os.path.join(parent_directory, "Reference.json"), 'r', encoding='utf-8') as file:
                references = json.load(file)
            if trans:
                response_cn = format_json_string(os.path.join(parent_directory, "images_insert_cn.json"))
    ...
    with open(os.path.join(parent_directory,"images_insert_optimal.txt"), 'w',encoding="utf-8") as file:
