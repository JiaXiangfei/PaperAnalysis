...
def Crop_image_Save(element, pageObj, pagenum, image_num,file = None,keep_temp = False,Save_path = "./"):
    ...
    if file is not None:
        ...
        if not os.path.exists(os.path.join(dict, filename)):
            os.makedirs(os.path.join(dict,filename))
        cropped_pdf_file_path = f'./temp/{filename}/cropped_image_{pagenum}_{image_num}.pdf'
        if not os.path.exists(os.path.join(Save_path, filename,"images")):
            os.makedirs(os.path.join(Save_path,filename,"images"))
        output_file = os.path.join(Save_path,filename,"images",f"page_{pagenum}_image_{image_num}.png")
    else:
        cropped_pdf_file_path = f'./temp/cropped_image_{pagenum}_{image_num}.pdf'
        output_file = os.path.join(Save_path,f"page_{pagenum}_image_{image_num}.png")
    ...
    if not keep_temp:
        ...
        if file is not None and len(os.listdir(os.path.join(dict, filename))) == 0:
            os.rmdir(os.path.join(dict, filename))
...
def File_Get_Image(file_path,store_path):
    ...
    if file_list > 0:
        for file in file_list:
            Get_Image(os.path.join(file_path,file), store_path)
